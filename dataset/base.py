...
class TrainDataset(BaseDataset):
    def __init__(self, root_dir, domains, tasks, shot, tasks_per_batch, domains_per_batch,
                 image_augmentation, unary_augmentation, binary_augmentation, mixed_augmentation,
                 dset_size=-1, trivial_augmentation=False, order_mixup=False, **kwargs):
        ...
        if unary_augmentation:
            if trivial_augmentation:
                self.unary_augmentation = CustomTrivialAugmentWide()
            else:
        ...
        if binary_augmentation is not None:
            self.binary_augmentation = Mixup(order=order_mixup)
        else:
...
class ContinuousDataset(BaseDataset):
    def __init__(self, root_dir, domains, task, channel_idx=-1, dset_size=-1, **kwargs):
        ...
        self.dset_size = dset_size
        self.n_channels = len(Unified.TASK_GROUP_DICT[task])
