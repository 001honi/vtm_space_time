...
class CustomBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, drop=0., attn_drop=0.,
                 drop_path=0., act_layer=nn.GELU, norm_layer=nn.LayerNorm, time_attention=True, n_frames=8):
        super().__init__()
    ...
    def forward(self, x):
        if self.time_attention:
            ...
            x = x + self.drop_path(self.attn_t(self.norm0(x)))
            x = x + self.temp_fc(x)
...
class CustomVisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
                 ...
                 act_layer=None, weight_init='', time_attention=True, n_frames=8):
        """
        Args:
        ...
        self.blocks = nn.Sequential(*[
            CustomBlock(
                ...
                time_attention=time_attention, n_frames=n_frames)
    ...
    def forward_features(self, x, t_idx=None):
...
@torch.no_grad()
def _load_weights(model: CustomVisionTransformer, checkpoint_path: str, prefix: str = ''):
...
def _create_vision_transformer(variant, pretrained=False, default_cfg=None, **kwargs):
    ...
    model = build_model_with_cfg(
        CustomVisionTransformer, variant, pretrained,
